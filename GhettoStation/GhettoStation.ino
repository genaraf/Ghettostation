...
extern void lcddisp_testservo(int testNum, int pan, int tilt);
...
void setup() {
  ...
  EEPROM_read(0, globalConfig);
  //clear eeprom & write default parameters if config is empty or wrong
  if ((globalConfig.config_crc != CONFIG_VERSION) || (globalConfig.current_bank > 3))
  {
    clear_eeprom();
    delay(20);
  }
  current_bank = globalConfig.current_bank;
  
  analogReference(INTERNAL2V56);

  pinMode(INPUT_SEL0, OUTPUT);
  pinMode(INPUT_SEL1, OUTPUT);
  
  // set temp value for servo pwm config
  servoconf_tmp[0] = globalConfig.pan_minpwm;
  servoconf_tmp[1] = globalConfig.pan_maxpwm;
  servoconf_tmp[2] = globalConfig.tilt_minpwm;
  servoconf_tmp[3] = globalConfig.tilt_maxpwm;
  voltage_ratio = (float)globalConfig.voltage_ratio / 100.0;

  EEPROM_read(config_bank[int(globalConfig.current_bank)], configuration);
  home_bearing = configuration.bearing; // use last bearing position of previous session.
  selectTelemInput(configuration.input);
  delay(20);
  ...
  // attach servos
  attach_servo(pan_servo, PAN_SERVOPIN, globalConfig.pan_minpwm, globalConfig.pan_maxpwm);
  attach_servo(tilt_servo, TILT_SERVOPIN, globalConfig.tilt_minpwm, globalConfig.tilt_maxpwm);
  ...
  switch (current_activity)
  {
    ...
    case 3:             //PAN_MINPWM
      ...
      if (servoconf_tmp[0] != servoconfprev_tmp[0]) {
        ...
        attach_servo(pan_servo, PAN_SERVOPIN, servoconf_tmp[0], globalConfig.pan_maxpwm);
      }
      pan_servo.writeMicroseconds(servoconf_tmp[0]);
      //pan_servo.write(0);
      servoconfprev_tmp[0] = servoconf_tmp[0];
      if (enter_button.holdTime() >= 700 && enter_button.held()) {//long press
        globalConfig.pan_minpwm = servoconf_tmp[0];
        EEPROM_write(0, globalConfig);
        detach_servo(pan_servo);
        attach_servo(pan_servo, PAN_SERVOPIN, globalConfig.pan_minpwm, globalConfig.pan_maxpwm);
        move_servo(pan_servo, 1, 0, globalConfig.pan_minangle, globalConfig.pan_maxangle);
        current_activity = 0;
      }
      break;
    case 4:             //PAN_MINANGLE
      globalConfig.pan_minangle = config_servo(1, 2, globalConfig.pan_minangle);
      pan_servo.writeMicroseconds(globalConfig.pan_minpwm);
      //pan_servo.write(0);
      if (enter_button.holdTime() >= 700 && enter_button.held()) {//long press
        EEPROM_write(0, globalConfig);
        move_servo(pan_servo, 1, 0, globalConfig.pan_minangle, globalConfig.pan_maxangle);
    ...
    case 5:             //PAN_MAXPWM
      ...
      if (servoconf_tmp[1] != servoconfprev_tmp[1]) {
        detach_servo(pan_servo);
        attach_servo(pan_servo, PAN_SERVOPIN, globalConfig.pan_minpwm, servoconf_tmp[1]);
      }
      pan_servo.writeMicroseconds(servoconf_tmp[1]);
      //pan_servo.write(180);
      servoconfprev_tmp[1] = servoconf_tmp[1];
      if (enter_button.holdTime() >= 700 && enter_button.held()) {//long press
        globalConfig.pan_maxpwm = servoconf_tmp[1];
        EEPROM_write(0, globalConfig);
        detach_servo(pan_servo);
        attach_servo(pan_servo, PAN_SERVOPIN, globalConfig.pan_minpwm, globalConfig.pan_maxpwm);
        move_servo(pan_servo, 1, 0, globalConfig.pan_minangle, globalConfig.pan_maxangle);
    ...
    case 6:             //PAN_MAXANGLE
      globalConfig.pan_maxangle = config_servo(1, 4, globalConfig.pan_maxangle );
      pan_servo.writeMicroseconds(globalConfig.pan_maxpwm);
      //pan_servo.write(180);
      if (enter_button.holdTime() >= 700 && enter_button.held()) {   //long press
        EEPROM_write(0, globalConfig);
        move_servo(pan_servo, 1, 0, globalConfig.pan_minangle, globalConfig.pan_maxangle);
    ...
    case 7:             //"TILT_MINPWM"
      ...
      if (servoconf_tmp[2] != servoconfprev_tmp[2]) {
        ...
        attach_servo(tilt_servo, TILT_SERVOPIN, servoconf_tmp[2], globalConfig.tilt_maxpwm);
      }
      tilt_servo.writeMicroseconds(servoconf_tmp[2]);
      //tilt_servo.write(0);
      servoconfprev_tmp[2] = servoconf_tmp[2];
      if (enter_button.holdTime() >= 700 && enter_button.held()) {    //long press
        globalConfig.tilt_minpwm = servoconf_tmp[2];
        EEPROM_write(0, globalConfig);
        detach_servo(tilt_servo);
        attach_servo(tilt_servo, TILT_SERVOPIN, globalConfig.tilt_minpwm, globalConfig.tilt_maxpwm);
        move_servo(tilt_servo, 2, 0, globalConfig.tilt_minangle, globalConfig.tilt_maxangle);;
        current_activity = 0;
      }
      break;
    case 8:             //TILT_MINANGLE
      globalConfig.tilt_minangle = config_servo(2, 2, globalConfig.tilt_minangle );
      tilt_servo.writeMicroseconds(globalConfig.tilt_minpwm);
      //tilt_servo.write(0);
      if (enter_button.holdTime() >= 700 && enter_button.held()) {//long press
        EEPROM_write(0, globalConfig);
        move_servo(tilt_servo, 2, 0, globalConfig.tilt_minangle, globalConfig.tilt_maxangle);
    ...
    case 9:             //"TILT_MAXPWM"
      ...
      if (servoconf_tmp[3] != servoconfprev_tmp[3]) {
        detach_servo(tilt_servo);
        attach_servo(tilt_servo, TILT_SERVOPIN, globalConfig.tilt_minpwm, servoconf_tmp[3]);
      }
      tilt_servo.writeMicroseconds(servoconf_tmp[3]);
      //tilt_servo.write(180);
      servoconfprev_tmp[3] = servoconf_tmp[3];
      if (enter_button.holdTime() >= 700 && enter_button.held()) {//long press
        globalConfig.tilt_maxpwm = servoconf_tmp[3];
        EEPROM_write(0, globalConfig);
        detach_servo(tilt_servo);
        attach_servo(tilt_servo, TILT_SERVOPIN, globalConfig.tilt_minpwm, globalConfig.tilt_maxpwm);
        move_servo(tilt_servo, 2, 0, globalConfig.tilt_minangle, globalConfig.tilt_maxangle);
    ...
    case 10:                //TILT_MAXANGLE
      globalConfig.tilt_maxangle = config_servo(2, 4, globalConfig.tilt_maxangle );
      tilt_servo.writeMicroseconds(globalConfig.tilt_maxpwm);
      //tilt_servo.write(180);
      if (enter_button.holdTime() >= 700 && enter_button.held()) {//long press
        EEPROM_write(0, globalConfig);
        move_servo(tilt_servo, 2, 0, globalConfig.tilt_minangle, globalConfig.tilt_maxangle);
    ...
    case 12:                //Configure Telemetry
      lcddisp_telemetry();
      if (enter_button.holdTime() >= 700 && enter_button.held()) { //long press
        EEPROM_write(config_bank[int(globalConfig.current_bank)], configuration);
        current_activity = 0;
      }
      break;
    ...
    case 14:                //Change settings bank
      lcddisp_bank();
      if (enter_button.holdTime() >= 700 && enter_button.held()) { //long press
        globalConfig.current_bank = current_bank;
        EEPROM_write(0, globalConfig);
    ...
    case 17:               //Configure voltage multiplier
      ...
      if (enter_button.holdTime() >= 700 && enter_button.held()) { //long press
        globalConfig.voltage_ratio = (uint16_t)(voltage_ratio * 100.0f);
        EEPROM_write(0, globalConfig);
        current_activity = 0;
      }
      break;
      
    case 18:                //Configure telemetry input
      lcddisp_telem_input();
      if (enter_button.holdTime() >= 700 && enter_button.held()) { //long press
        EEPROM_write(config_bank[int(globalConfig.current_bank)], configuration);
        selectTelemInput(configuration.input);
...
void leftButtonReleaseEvents(Button &btn)
{
  if ( left_button.holdTime() < 700 ) {
    ...
    else if ( current_activity != 0 && current_activity != 2 ) {
      //We're in a setting area: Left button decrase current value.
      switch (current_activity) {
        case 1:   if(tracking_scr_num > 0) tracking_scr_num--; break;
        case 3:   servoconf_tmp[0]--;            break;
        case 4:   globalConfig.pan_minangle--;  break;
        case 5:   servoconf_tmp[1]--;            break;
        case 6:   globalConfig.pan_maxangle--;  break;
        case 7:   servoconf_tmp[2]--;            break;
        case 8:   globalConfig.tilt_minangle--; break;
        case 9:   servoconf_tmp[3]--;            break;
        case 10:  globalConfig.tilt_maxangle--; break;
        ...
        case 18:  if (configuration.input > 0) configuration.input--; break;
      }
    }
    else if (current_activity == 2) {
...
void rightButtonReleaseEvents(Button &btn)
{
  if ( right_button.holdTime() < 700 ) {
    ...
    if (current_activity == 0) {
      ...
      switch (current_activity) {
        case 1:  if(tracking_scr_num < 3) tracking_scr_num++; break;
        case 3:  servoconf_tmp[0]++;            break;
        case 4:  globalConfig.pan_minangle++;  break;
        case 5:  servoconf_tmp[1]++;            break;
        case 6:  globalConfig.pan_maxangle++;  break;
        ...
        case 18: if(configuration.input < 3) configuration.input++; break;
     }
    }
    else if (current_activity == 2) {
      if (configuration.bearing_method == 2) {
...
void init_menu() {
  ...
  m1m3m2Menu.add_item(&m1m3m2i3Item, &configure_input); // select telemetry input
...
void configure_input(MenuItem* p_menu_item) {
  current_activity = 18;
}
...
void selectTelemInput(int inNum)
{
  switch(inNum)
  {
    case 0:
      digitalWrite(INPUT_SEL0, 0);  
      digitalWrite(INPUT_SEL1, 0);  
      break;
    case 1:
      digitalWrite(INPUT_SEL0, 1);  
      digitalWrite(INPUT_SEL1, 0);  
      break;
    case 2:
      digitalWrite(INPUT_SEL0, 0);  
      digitalWrite(INPUT_SEL1, 1);  
      break;
    case 3:
      digitalWrite(INPUT_SEL0, 1);  
      digitalWrite(INPUT_SEL1, 1);  
...
void move_servo(PWMServo &s, int stype, int a, int mina, int maxa) {
  ...
  if (stype == 1) {
    ...
    int microsec = map(a, 0, mina + maxa, globalConfig.pan_minpwm, globalConfig.pan_maxpwm);
    s.writeMicroseconds( microsec );
  }
  else if (stype == 2) {
    //map angle to microseconds
    int microsec = map(a, mina, maxa, globalConfig.tilt_minpwm, globalConfig.tilt_maxpwm);
    s.writeMicroseconds( microsec );
  }
}
...
void servoPathfinder(int angle_b, int angle_a) {  // ( bearing, elevation )
  ...
  if (angle_b <= 180) {
    if ( globalConfig.pan_maxangle >= angle_b ) {
      //define limits
      if (angle_a <= globalConfig.tilt_minangle) {
        // checking if we reach the min tilt limit
        angle_a = globalConfig.tilt_minangle;
      } else if (angle_a > globalConfig.tilt_maxangle) {
        //shouldn't happend but just in case
        angle_a = globalConfig.tilt_maxangle;
      }
    } else if ( globalConfig.pan_maxangle < angle_b ) {
      ...
      // invert pan axis
      if ( globalConfig.tilt_maxangle >= ( 180 - angle_a )) {
        // invert pan & tilt for 180° Pan 180° Tilt config
        angle_a = 180 - angle_a;
      }
      else if (globalConfig.tilt_maxangle < ( 180 - angle_a )) {
    ...
    if ( globalConfig.pan_minangle > 360 - angle_b ) {
      if (angle_a < globalConfig.tilt_minangle) {
        // checking if we reach the min tilt limit
        angle_a = globalConfig.tilt_minangle;
      }
    } else if ( globalConfig.pan_minangle <= 360 - angle_b ) {
      angle_b = angle_b - 180;
      if ( globalConfig.tilt_maxangle >= ( 180 - angle_a )) {
  ...
  move_servo(pan_servo, 1, angle_b, globalConfig.pan_minangle, globalConfig.pan_maxangle);
  move_servo(tilt_servo, 2, angle_a, globalConfig.tilt_minangle, globalConfig.tilt_maxangle);
}
...
void test_servos() {
  ...
  lcddisp_testservo(test_servo_step, p, t);
//  move_servo(pan_servo, 1, p, globalConfig.pan_minangle, globalConfig.pan_maxangle);
//  move_servo(tilt_servo, 2, t, globalConfig.tilt_minangle, globalConfig.tilt_maxangle);
  servoPathfinder(p, t);
...
void read_voltage() {
  voltage_actual = (analogRead(ADC_VOLTAGE) * ANALOG_REFERENCE / 1024.0) * voltage_ratio;
